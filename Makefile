...
.PHONY: help up down restart build rebuild logs logs-php logs-httpd logs-mysql logs-phpmyadmin status shell-php shell-httpd shell-mysql clean clean-all info test check-files composer-install composer-update composer-require dev-reset
...
help: ## Показать справку по командам
	@echo "$(YELLOW)Guest Book (PHP 8.4)$(NC)"
	@echo "======================================"
	@echo ""
...
check-files: ## Проверить наличие всех необходимых файлов
	...
	@test -d src/public/ || (echo "$(RED)✗ директория src/public/ не найдена$(NC)" && exit 1)
...
logs-php: ## Просмотр логов PHP-FPM
	docker compose logs -f php-guest-book

logs-httpd: ## Просмотр логов Apache
	docker compose logs -f httpd-guest-book

logs-mysql: ## Просмотр логов MySQL
	docker compose logs -f mysql-guest-book

logs-phpmyadmin: ## Просмотр логов phpMyAdmin
	docker compose logs -f phpmyadmin-guest-book
...
shell-php: ## Подключиться к контейнеру PHP
	docker compose exec php-guest-book sh

shell-httpd: ## Подключиться к контейнеру Apache
	docker compose exec httpd-guest-book sh

shell-mysql: ## Подключиться к MySQL CLI
	docker compose exec mysql-guest-book mysql -u root -p

info: ## Показать информацию о проекте
	@echo "$(YELLOW)Guest Book$(NC)"
	@echo "======================================"
	...
	@echo "  • src/public/       - публичная директория"
...
composer-install: ## Установить зависимости через Composer
	docker compose exec php-guest-book composer install

composer-update: ## Обновить зависимости через Composer
	docker compose exec php-guest-book composer update

composer-require: ## Установить пакет через Composer (make composer-require PACKAGE=vendor/package)
	docker compose exec php-guest-book composer require $(PACKAGE)
